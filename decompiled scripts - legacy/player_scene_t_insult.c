...
void func_16() // Position - 0x836 (2102)
{
	...
	if (ENTITY::DOES_ENTITY_EXIST(PLAYER::PLAYER_PED_ID()))
	{
		if (_GET_CHARACTER_MODEL(Global_114344.f_2367.f_539.f_4321) != ENTITY::GET_ENTITY_MODEL(PLAYER::PLAYER_PED_ID()))
		{
			...
			if (func_17(character) && !func_21(14) || Global_113292)
			{
		...
		if (PED::IS_SYNCHRONIZED_SCENE_RUNNING(Global_99198))
		{
			for (i = 0; i < iLocal_236; i = i + 1)
			{
				if (!PED::IS_PED_INJURED(iLocal_236[i]))
				{
					...
					TASK::TASK_SYNCHRONIZED_SCENE(iLocal_236[i], Global_99198, &uLocal_344, &uLocal_393[i /*16*/], 1000f, blendOut, flags, ragdollBlockingFlags, moverBlendDelta, ikFlags);
					iLocal_239[i] = 0;
				}
			}
			...
			for (i = 0; i < iLocal_252; i = i + 1)
			{
				if (ENTITY::DOES_ENTITY_EXIST(iLocal_252[i]))
					if (!MISC::IS_STRING_NULL_OR_EMPTY(&uLocal_558[i /*16*/]))
						ENTITY::PLAY_SYNCHRONIZED_ENTITY_ANIM(iLocal_252[i], Global_99198, &uLocal_558[i /*16*/], &uLocal_344, 8f, -1000f, 0, 1148846080);
			}
...
BOOL func_38() // Position - 0x1C65 (7269)
{
	...
	if (PED::IS_SYNCHRONIZED_SCENE_RUNNING(Global_99197))
	{
		...
		{
			if (!PED::IS_PED_INJURED(iLocal_236[i]))
				TASK::TASK_SYNCHRONIZED_SCENE(iLocal_236[i], Global_99197, &uLocal_344, &uLocal_360[i /*16*/], 8f, -1000f, 0, 0, 1148846080, 0);
		}
		...
		for (i = 0; i < iLocal_252; i = i + 1)
		{
			if (ENTITY::DOES_ENTITY_EXIST(iLocal_252[i]))
				if (!MISC::IS_STRING_NULL_OR_EMPTY(&uLocal_525[i /*16*/]))
					ENTITY::PLAY_SYNCHRONIZED_ENTITY_ANIM(iLocal_252[i], Global_99197, &uLocal_525[i /*16*/], &uLocal_344, 8f, -1000f, 0, 1148846080);
		}
...
int func_39(int iParam0, Ped pedParam1) // Position - 0x1D61 (7521)
{
	...
	if (!STREAMING::IS_PLAYER_SWITCH_IN_PROGRESS())
	{
		...
		else if (pedParam1 == func_40(Global_101614) && pedParam1 != 0)
		{
			Global_101614 = 0;
		}
	}
	...
	switch (iParam0)
	{
		...
		case 5:
			*uParam1 = Global_114344.f_2367.f_539.f_2310[0];
			TEXT_LABEL_ASSIGN_STRING(sParam2, func_57(Global_114344.f_2367.f_539.f_2314[0]), 32);
			return true;
	
		case 6:
			*uParam1 = Global_114344.f_2367.f_539.f_2310[1];
			TEXT_LABEL_ASSIGN_STRING(sParam2, func_57(Global_114344.f_2367.f_539.f_2314[1]), 32);
			return true;
	
		case 7:
			*uParam1 = Global_114344.f_2367.f_539.f_2310[2];
			TEXT_LABEL_ASSIGN_STRING(sParam2, func_57(Global_114344.f_2367.f_539.f_2314[2]), 32);
			return true;
