			...
			if (func_50(PLAYER::PLAYER_PED_ID(), &uParam0->[uParam0->f_57.f_2 /*14*/]))
			{
				...
				else if (uParam0->f_57.f_5 == -1)
				{
					if (MISC::GET_GAME_TIMER() - uParam0->f_57.f_6 > 150)
						_CONTEXT_ADD_HELP_TEXT(&(uParam0->f_57.f_5), 4, "MPJAC_SIT" /*Presiona ~INPUT_CONTEXT~ para sentarte.*/, 0, 0, 0, 0);
				}
		...
		else
		{
			...
			if (!HUD::IS_PAUSE_MENU_ACTIVE() && !func_8(&(uParam0->f_57), 0) && !func_8(&(uParam0->f_57), 4) && !func_21() && !func_19(true) && uParam0->f_57.f_8.f_2 != 3)
			{
				if (uParam0->f_57.f_5 == -1)
					_CONTEXT_ADD_HELP_TEXT(&(uParam0->f_57.f_5), 4, "MPJAC_EXIT" /*Presiona ~INPUT_CONTEXT~ para ponerte de pie.*/, 0, 0, 0, 0);
...
void func_71() // Position - 0x184E (6222)
{
	...
	unk[0] = 0;
	unk[1] = 1;
	func_72(5, &uLocal_129, &unk);
	unk[0] = 2;
	unk[1] = 3;
	func_72(6, &uLocal_129, &unk);
	return;
}
...
BOOL func_82() // Position - 0x1D6F (7535)
{
	if (func_83())
		return true;
...
BOOL func_83() // Position - 0x1D8A (7562)
{
